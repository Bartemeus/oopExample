...
namespace Smartcontract.App.Infrastructure.Services.Implementation.Sms.SmsCenterKz {
    public class SMSC {
        private readonly string SMSC_LOGIN; // логин клиента
        private readonly string SMSC_PASSWORD; // пароль или MD5-хеш пароля в нижнем регистре
        ...
        public string[][] D2Res;
        private const string SmsEventId = "SMS";
        ...
        public string[] send_sms(string phones, string message, int translit = 0, string time = "", int id = 0, int format = 0, string sender = "", string query = "", string[] files = null) {
            ...
            if (SMSC_DEBUG) {
                int code;
                if (m.Length > 1 && int.TryParse(m[1], out code)) {
                    if (code > 0) {
                        Log.Information("{EventId} ID:{smsId} sent to {@phones}: {smsText}", SmsEventId, m[0], phones, message);
                    }
                    else {
                        Log.Error("{EventId} ID:{smsId} sent to {@phones} error:{smsErrorCode}", SmsEventId, m[0], phones, code);
                    }
                }
            }
        ...
        public string[] get_sms_cost(string phones, string message, int translit = 0, int format = 0, string sender = "", string query = "") {
            ...
            if (SMSC_DEBUG) {
                var code = Convert.ToInt32(m[1]);
                if (code > 0) {
                    Log.Information("{EventId} ID:{smsId} sent to {@phones}: {smsText}", SmsEventId, m[0], phones, message);
                }
                else {
                    Log.Error("{EventId} ID:{smsId} error:{smsErrorCode}", SmsEventId, m[0], code);
                }


                if (code > 0)
                    Log.Information("{EventId} Стоимость рассылки. " + m[0] + ". Всего SMS: " + m[1], SmsEventId);
                else
                    Log.Information("{EventId} Стоимость рассылки. error:{smsErrorCode}", SmsEventId, code);
            }
            ...
            if (id.IndexOf(',') == -1) {
                if (SMSC_DEBUG) {
                    if (m[1] != "" && Convert.ToInt32(m[1]) >= 0) {
                        ...
                        Log.Information("Статус SMS = " + m[0] + (timestamp > 0 ? ", время изменения статуса - " + date.ToLocalTime() : ""));
                    }
                    else
                        Log.Warning("Ошибка №" + m[1].Substring(1, 1));
                }
        ...
        public string get_balance() {
            ...
            if (SMSC_DEBUG) {
                if (m.Length == 1)
                    Log.Information("Сумма на счете: " + m[0]);
                else
                    Log.Warning("Ошибка №" + m[1].Substring(1, 1));
            }
        ...
        private string[] _smsc_send_cmd(string cmd, string arg, string[] files = null) {
            ...
            if (ret == "") {
                if (SMSC_DEBUG)
                    Log.Warning("Ошибка чтения адреса: " + url);
